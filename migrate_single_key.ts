...
for await (const flag of flagData.items) {
  if (flag.key == inputArgs.key) {
    ...
    if (flagResp.status == 200 || flagResp.status == 201) {
      ...
      if (flag.deprecated) {
        console.log("Flag ${newFlag.key} is deprecated, patching to reflect the state");
        const patchReq: any[] = [];
        patchReq.push(
          buildPatch(
            `/deprecated`,
...
for await (const flag of flagData.items) {
  if (flag.key == inputArgs.key) {
    console.log("Managing flag " + flag.key);
...
async function makePatchCall(flagKey, patchReq){
  ...
  if (flagPatchStatus == 400) {
    ...
    patchFlagReq.json().then((data) => {console.log(data)})
    console.log('failing request is: ' + JSON.stringify(patchReq)) 
  }
